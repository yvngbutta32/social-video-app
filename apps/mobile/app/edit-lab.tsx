  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {confirmingRender && serverVariantId ? <CreatorCard style={styles.confirmCard}><View style={styles.confirmHeading}><View style={styles.confirmIcon}><IconSymbol name="lock.shield.fill" size={18} color={colors.primary} /></View><View style={styles.confirmCopy}><Eyebrow>Private render handoff</Eyebrow><Text style={[styles.confirmTitle, { color: colors.foreground }]}>Queue this revision for private rendering?</Text></View></View><Text style={[styles.copy, { color: colors.muted }]}>This saves the current non-destructive recipe for {clipPlatform}. {recipe.selectedClipCandidateId ? "The selected clip candidate will be included as provenance." : "No clip candidate is selected; the current manual trim will be used."} The result stays in the workspace and is not published.</Text><View style={styles.confirmActions}><Pressable accessibilityRole="button" onPress={() => setConfirmingRender(false)} style={({ pressed }) => [styles.confirmSecondary, { borderColor: colors.border }, pressed && styles.pressed]}><Text style={[styles.confirmSecondaryText, { color: colors.foreground }]}>Not yet</Text></Pressable><Pressable accessibilityRole="button" onPress={() => void save()} style={({ pressed }) => [styles.confirmPrimary, { backgroundColor: colors.primary }, pressed && styles.pressed]}><IconSymbol name="lock.shield.fill" size={16} color={colors.background} /><Text style={[styles.confirmPrimaryText, { color: colors.background }]}>Queue privately</Text></Pressable></View></CreatorCard> : null}
        <Pressable disabled={saving} onPress={() => { if (serverVariantId && !confirmingRender) { setConfirmingRender(true); return; } void save(); }} style={({ pressed }) => [styles.save, { backgroundColor: colors.primary }, (pressed || saving) && styles.pressed]}>{saving ? <ActivityIndicator color={colors.background} /> : <IconSymbol name="wand.and.stars" size={20} color={colors.background} />}<Text style={[styles.saveText, { color: colors.background }]}>{saving ? "Saving secure revision" : saved ? (serverVariantId ? "Revision saved and queued" : "Revision saved locally") : serverVariantId && !confirmingRender ? "Review & queue private render" : "Save edit revision"}</Text></Pressable>
      </ScrollView>
    </ScreenContainer>
  );
}
...
const styles = StyleSheet.create({
  confirmCard: { gap: 13, borderWidth: 1, borderColor: "#55E6FF55", backgroundColor: "#10283D" }, confirmHeading: { flexDirection: "row", alignItems: "center", gap: 10 }, confirmIcon: { width: 38, height: 38, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: "#55E6FF18" }, confirmCopy: { flex: 1, gap: 4 }, confirmTitle: { fontSize: 17, lineHeight: 22, fontWeight: "900" }, confirmActions: { flexDirection: "row", gap: 9 }, confirmSecondary: { flex: 1, minHeight: 46, borderRadius: 14, borderWidth: 1, alignItems: "center", justifyContent: "center" }, confirmSecondaryText: { fontSize: 13, fontWeight: "900" }, confirmPrimary: { flex: 1.35, minHeight: 46, borderRadius: 14, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 6 }, confirmPrimaryText: { fontSize: 13, fontWeight: "900" },
