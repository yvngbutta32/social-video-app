...
export default function ReviewScreen() {
  ...
  const { selectedSourceId, sources, recipeFor, platformsFor, setPlatformTargets, briefFor, setAdaptationBrief } = useCreatorWorkflow();
  const source = sources.find((item) => item.id === selectedSourceId) ?? null;
  const selectedPlatforms = platformsFor(source?.id ?? "");
  const creatorBrief = briefFor(source?.id ?? "");
  ...
  const refreshPlan = useCallback(async () => {
    ...
    setLoadingPlan(true);
    setNotice(null);
    try { setPlan(await prepareAdaptationPlan(source.serverVideoId, selectedPlatforms, creatorBrief)); }
  ...
  }, [creatorBrief, selectedPlatforms, source?.serverVideoId, source?.status]);

  useEffect(() => { if (!creatorBrief) void refreshPlan(); }, [creatorBrief, refreshPlan]);
  useEffect(() => { setBriefDraft(creatorBrief); }, [creatorBrief, source?.id]);
  ...
  const saveBrief = () => {
    setAdaptationBrief(source.id, briefDraft);
    setPlan(null);
    setNotice("Creator direction saved. Refresh the verified plan to prepare new private drafts with it.");
  };
    ...
    <ScreenContainer className="px-5" edges={["top", "bottom", "left", "right"]}>
      <FlatList
        ...
        ListHeaderComponent={<>
          ...
          <CreatorCard style={[styles.sourceCard, { backgroundColor: "#0B2741", borderColor: "#2B6689" }]}>
            ...
            <View style={styles.briefCard}>
              <View style={styles.briefHeading}><View><Text style={styles.briefEyebrow}>CREATOR DIRECTION</Text><Text style={styles.briefTitle}>What should this version emphasize?</Text></View><Text style={styles.briefCount}>{briefDraft.length}/280</Text></View>
              <TextInput value={briefDraft} onChangeText={setBriefDraft} placeholder="For example: Keep the proof practical for first-time founders." placeholderTextColor="#6F96AE" multiline maxLength={280} textAlignVertical="top" accessibilityLabel="Creator direction for private adaptation drafts" accessibilityHint="Saved direction shapes the next private adaptation plan. It does not publish content or control reach." style={styles.briefInput} />
              <Pressable onPress={saveBrief} disabled={briefDraft.trim() === creatorBrief} accessibilityRole="button" accessibilityLabel="Save creator direction" accessibilityHint="Saves private editorial direction for this source. Refresh the plan to use it." style={({ pressed }) => [styles.briefSave, briefDraft.trim() === creatorBrief && styles.briefSaveDisabled, pressed && styles.pressed]}><Text style={styles.briefSaveText}>{creatorBrief ? "Update direction" : "Save direction"}</Text></Pressable>
              <Text style={styles.briefHint}>This is private editorial direction for drafts. It does not instruct a platform, publish, or guarantee an outcome.</Text>
            </View>
...
const styles = StyleSheet.create({ content: { gap: 16, paddingTop: 22, paddingBottom: 30 }, header: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 4 }, iconButton: { width: 44, height: 44, borderRadius: 15, borderWidth: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#13273B" }, headerCopy: { flex: 1, gap: 3 }, sourceName: { fontSize: 18, fontWeight: "800", letterSpacing: -0.25 }, sourceCard: { gap: 14, padding: 21, shadowColor: "#000000", shadowOpacity: 0.32, shadowRadius: 22, elevation: 8 }, sourceCardTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, sourceCardTitle: { color: "#F1FAFF", fontSize: 25, lineHeight: 31, fontWeight: "800", letterSpacing: -0.6 }, sourceCardCopy: { color: "#BDD7E8", fontSize: 14, lineHeight: 22 }, briefCard: { gap: 9, backgroundColor: "#061D32", borderWidth: 1, borderColor: "#326481", borderRadius: 16, padding: 13 }, briefHeading: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }, briefEyebrow: { color: "#55E6FF", fontSize: 9, fontWeight: "900", letterSpacing: 1 }, briefTitle: { color: "#F1FAFF", fontSize: 14, fontWeight: "800", marginTop: 3 }, briefCount: { color: "#94C2D8", fontSize: 11, fontWeight: "800", paddingTop: 2 }, briefInput: { minHeight: 76, color: "#F1FAFF", fontSize: 14, lineHeight: 20, borderWidth: 1, borderColor: "#39708F", backgroundColor: "#08192A", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10 }, briefSave: { minHeight: 38, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: "#1E6687" }, briefSaveDisabled: { backgroundColor: "#14384F", opacity: 0.72 }, briefSaveText: { color: "#E9FAFF", fontSize: 13, fontWeight: "900" }, briefHint: { color: "#94C2D8", fontSize: 11, lineHeight: 16 }, targetRow: { flexDirection: "row", flexWrap: "wrap", gap: 8 }, targetChip: { minHeight: 40, borderRadius: 13, borderWidth: 1, paddingHorizontal: 12, justifyContent: "center" }, targetChipText: { color: "#F1FAFF", fontSize: 12, fontWeight: "900", letterSpacing: -0.1 }, connectionPanel: { gap: 10, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: "#39708F", paddingTop: 14 }, connectionPanelHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, connectionEyebrow: { color: "#94C2D8", fontSize: 9, fontWeight: "900", letterSpacing: 1.1 }, connectionRefresh: { minHeight: 30, paddingHorizontal: 6, justifyContent: "center" }, connectionRefreshText: { color: "#55E6FF", fontSize: 12, fontWeight: "900" }, connectionRow: { gap: 4, paddingVertical: 2 }, connectionPlatform: { color: "#F1FAFF", fontSize: 13, fontWeight: "900" }, connectionCopy: { gap: 3 }, connectionLabel: { fontSize: 12, fontWeight: "900" }, connectionDetail: { color: "#BDD7E8", fontSize: 12, lineHeight: 18 }, requirementsBlock: { gap: 4, marginTop: 7, borderLeftWidth: 2, borderLeftColor: "#39708F", paddingLeft: 9 }, requirementsEyebrow: { color: "#94C2D8", fontSize: 9, fontWeight: "900", letterSpacing: 0.9 }, requirementText: { color: "#D7EAF5", fontSize: 12, lineHeight: 18 }, connectionUnavailable: { color: "#BDD7E8", fontSize: 12, lineHeight: 18 }, planButton: { height: 50, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, shadowColor: "#55E6FF", shadowOpacity: 0.2, shadowRadius: 10, elevation: 4 }, planButtonText: { color: "#0B2741", fontSize: 14, fontWeight: "900" }, notice: { fontSize: 13, fontWeight: "700", lineHeight: 19 }, section: { gap: 5, marginTop: 4 }, sectionTitle: { fontSize: 22, lineHeight: 28, fontWeight: "800", letterSpacing: -0.4 }, appliedBrief: { color: "#8EC5DC", fontSize: 12, lineHeight: 18, marginTop: 2 }, adaptationCard: { gap: 14, padding: 19 }, cardHeader: { flexDirection: "row", alignItems: "center", gap: 11 }, platformGlyph: { width: 46, height: 46, borderRadius: 16, alignItems: "center", justifyContent: "center" }, platformCopy: { flex: 1, gap: 4 }, platformName: { fontSize: 17, fontWeight: "800", letterSpacing: -0.2 }, platformFormat: { fontSize: 12, fontWeight: "800" }, focus: { fontSize: 16, lineHeight: 23, fontWeight: "700", letterSpacing: -0.1 }, caption: { fontSize: 13, lineHeight: 20 }, status: { fontSize: 13, lineHeight: 20 }, refine: { height: 48, borderRadius: 15, borderWidth: 1, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 8, backgroundColor: "#13273B" }, refineText: { fontSize: 14, fontWeight: "900" }, preview: { height: 48, borderRadius: 15, alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 8, shadowColor: "#55E6FF", shadowOpacity: 0.14, shadowRadius: 9, elevation: 3 }, previewText: { fontSize: 14, fontWeight: "900" }, evidence: { flexDirection: "row", gap: 12, alignItems: "flex-start", backgroundColor: "#0C1B2D" }, evidenceIcon: { width: 40, height: 40, borderRadius: 14, alignItems: "center", justifyContent: "center" }, evidenceCopy: { flex: 1, gap: 4 }, evidenceTitle: { fontSize: 14, fontWeight: "900" }, evidenceText: { fontSize: 13, lineHeight: 20 }, empty: { fontSize: 20, lineHeight: 28, fontWeight: "800" }, back: { marginTop: 16, height: 50, borderRadius: 15, borderWidth: 1, alignItems: "center", justifyContent: "center", backgroundColor: "#13273B" }, pressed: { opacity: 0.78, transform: [{ scale: 0.985 }] } });
