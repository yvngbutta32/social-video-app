...
export default function ReviewScreen() {
  ...
  const refreshPlan = useCallback(async () => {
    if (!source?.serverVideoId || source.status !== "ready") return;
    setLoadingPlan(true);
    setNotice(null);
    try { setPlan(await prepareAdaptationPlan(source.serverVideoId)); }
  ...
  }, [source?.serverVideoId, source?.status]);

  useEffect(() => { void refreshPlan(); }, [refreshPlan]);

  const drafts = useMemo<ReviewDraft[]>(() => plan?.experiments.map((experiment) => ({ key: experiment.variantId, variantId: experiment.variantId, platform: formatPlatform(experiment.platform), format: experiment.aspectRatio, focus: experiment.hook, caption: experiment.caption, status: experiment.availability.replaceAll("_", " ") })) ?? localBlueprints, [plan]);
      ...
      <FlatList
        ...
        ListHeaderComponent={<>
          ...
          <CreatorCard style={[styles.sourceCard, { backgroundColor: "#123B57", borderColor: "#2A6586" }]}>
            <View style={styles.sourceCardTop}><IconSymbol name="wand.and.stars" size={25} color="#55E6FF" /><StatusPill tone={plan ? "ready" : sourceReadyForPlan ? "accent" : "attention"}>{plan ? "VERIFIED PLAN READY" : sourceReadyForPlan ? "PREPARING PLAN" : source.serverVideoId ? "PROCESSING REQUIRED" : "PRIVATE QUEUE NOT CONNECTED"}</StatusPill></View>
            <Text style={styles.sourceCardTitle}>{plan ? "Your server-verified adaptation plan" : "Your platform draft blueprint"}</Text>
            <Text style={styles.sourceCardCopy}>{plan ? plan.nextStep : `The current local recipe uses ${formatDuration(draftLength * 1000)} from ${recipe.trimStartSeconds}s to ${recipe.trimEndSeconds}s with ${recipe.composition.replace("_", " ")} composition. Private processing is required before an adaptation plan or artifact exists.`}</Text>
            {sourceReadyForPlan ? <Pressable onPress={() => void refreshPlan()} disabled={loadingPlan} style={({ pressed }) => [styles.planButton, { backgroundColor: "#55E6FF" }, (pressed || loadingPlan) && styles.pressed]}>{loadingPlan ? <ActivityIndicator color="#123B57" /> : <><IconSymbol name="arrow.triangle.2.circlepath" size={18} color="#123B57" /><Text style={styles.planButtonText}>Refresh verified plan</Text></>}</Pressable> : null}
          </CreatorCard>
          {notice ? <Text style={[styles.notice, { color: colors.warning }]}>{notice}</Text> : null}
          <View style={styles.section}><Eyebrow>{plan ? "Verified adaptations" : "Planned adaptations"}</Eyebrow><Text style={[styles.sectionTitle, { color: colors.foreground }]}>{plan ? "Review server-recorded drafts." : "Review local intent, then refine."}</Text></View>
        </>}
        renderItem={({ item }) => <CreatorCard style={styles.adaptationCard}>
          ...
          <Pressable onPress={() => router.push("/edit-lab" as never)} style={({ pressed }) => [styles.refine, { borderColor: colors.border }, pressed && styles.pressed]}><IconSymbol name="slider.horizontal.3" size={19} color={colors.primary} /><Text style={[styles.refineText, { color: colors.foreground }]}>Refine local recipe</Text></Pressable>
