...
describe("native edit synchronization", () => {
  ...
  it("keeps local representation while applying the server-authoritative revision", () => {
    const recipe = serverRecipeToLocalRecipe("source-1", { sourceRange: { startSeconds: 2, endSeconds: 12 }, composition: { mode: "smart_fill", focalPoint: null }, captions: { enabled: false, style: "off" }, headline: null, headlinePlacement: "center_safe", audio: { normalize: false }, provenance: { revision: 3 } }, local);
    expect(recipe).toMatchObject({ composition: "smart_crop", headline: "", headlinePlacement: "center_safe", revision: 3, focalX: 0.4, focalY: 0.6 });
  });
});
